/**
 * Forked from https://github.com/dtjohnson/xlsx-populate/blob/0a63f33332b159f21970c80808151ac97f6a7d60/lib/xmlq.js
...
module.exports = {
     ...
     */
    findChild(node, targetName) {
        return node.children.find(({name}) => name === targetName);
    },
     ...
     */
    hasChild(node, name) {
        return node.children.some(({childName}) => childName === name);
    },
    ...
    removeChild(node, child) {
        ...
        if (typeof child === 'string') {
            node.children = node.children.filter((c) => c.name !== child);
        } else {
    ...
    setAttributes(node, attributes) {
        Object.getOwnPropertyNames((attribute) => {
